...
当你同时使用 Claude Code、Codex、Trae 等多个工具时，可以查看当前事件状态：

macOS/Linux:

```bash
python3.11 -m aicore.cli status
```

Windows PowerShell:

```powershell
py -3.11 -m aicore.cli status
```

输出会包含：

- `Sessions`：当前历史事件中出现过的 `session-id`
- `Pending Events`：已经 `log-write` 但还没有进入 checkpoint 的事件
- `Multi-session File Risks`：多个 session 修改过同一文件的风险提示

`status` 只读取 `.aicore/history` 推导状态，不新增第二套状态源。
